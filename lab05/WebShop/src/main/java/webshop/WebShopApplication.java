...
public class WebShopApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		retTemplate.postForObject("http://localhost:8080/product", productRequest, Void.class);

		// create product 2
		HttpEntity<ProductDTO> productRequest2 = new HttpEntity<>(new ProductDTO("2", 44, "Product2", "Product2"));
		retTemplate.postForObject("http://localhost:8080/product", productRequest2, Void.class);

		/// {productNumber}/review
		retTemplate.put("http://localhost:8080/product/1/review", new ReviewDTO("Review1", "This is Review #1 ", 9));
		retTemplate.put("http://localhost:8080/product/1/review", new ReviewDTO("Review2", "This is Review #2 ", 8));

		// set product 1 stock info (quantity:30, locationCode:777)
		retTemplate.put("http://localhost:8080/product/1/stock/30/777", null);

		// set product 2 stock info (quantity:20, locationCode:777)
		retTemplate.put("http://localhost:8080/product/2/stock/20/777", null);

		// add product 1 with quantity 2 to shopping card 1
		retTemplate.put("http://localhost:8080/shoppingcart/1/1/20", null);

		// add product 1 with quantity 3 to shopping card 1
		retTemplate.put("http://localhost:8080/shoppingcart/1/2/3", null);

		// checkout shopping card 1
		retTemplate.put("http://localhost:8080/shoppingcart/checkout/1", null, Void.class);

		// add initial customer with number 111
		customerService.addCustomer();

		// set customer 111 to order 1
		ResponseEntity<List<CustomerDTO>> exchange = retTemplate.exchange("http://localhost:8080/customer",
		...
		// set customer 111 to order 1
		retTemplate.put("http://localhost:8080/order/1/customer/111", null, Void.class);

		// set credit card information to order 1
		retTemplate.put("http://localhost:8080/order/1/creditcard", new CreditCardDTO("33366555333888", new Date()),
				Void.class);

		// set Shipping option to order 1
		retTemplate.put("http://localhost:8080/order/1/shippingoption",
				new ShippingOptionDTO("Option1", 55, "shipper1"), Void.class);

		// set Shipping address to order 1
		retTemplate.put("http://localhost:8080/order/1/shippingAddress",
				new AddressDTO("shipping address1", "city1", 555, "USA"), Void.class);

		// set billing address to order 1
		retTemplate.put("http://localhost:8080/order/1/billingAddress",
				new AddressDTO("billing address1", "city2", 888, "USA"), Void.class);

		// retrieve order 3
		OrderDTO order = retTemplate.getForObject("http://localhost:8080/order/1", OrderDTO.class);
		System.out.println("***** " + order);

		// confirm order 1
		retTemplate.put("http://localhost:8080/order/1/confirm", null, Void.class);
