...
public class ProductService {
	 ...
	 * 
	 * @param productnumber
	 * @return {@link webshop.shared.dto.ProductDTO}
	 */
	public webshop.shared.dto.ProductDTO getProduct(String productnumber) {
		Optional<Product> result = productRepository.findById(productnumber);
		if (result.isPresent()) {
			Product product = result.get();
			return new webshop.shared.dto.ProductDTO(product.getProductNumber(), product.getName(), product.getPrice());
		} else
			return null;
	}
	 ...
	 * @return {@link webshop.shared.dto.ProductDTO}
	 */
	public ProductWithStockDTO getProductWithStock(String productnumber) {
		Optional<Product> result = productRepository.findById(productnumber);
		if (result.isPresent()) {
			Product product = result.get();
			Stock stock = product.getStock();
			return new ProductWithStockDTO(product.getProductNumber(), product.getName(), product.getPrice(),
